...
def compute_offpolicy_seq_mask(
    ...
    """Compute Off-Policy Sequence Masking (DeepSeek-V3.2).

    In asynchronous RL training, rollout data is generated by an old policy π_old
    while the current policy π_θ has diverged significantly. When advantage < 0
    (bad sample) and the old policy assigns much higher probability than the
    current policy (π_old >> π_θ, i.e. KL(π_old || π_θ) > δ), the gradient
    from such samples can be misleading — the current policy is being penalized
    for actions it would no longer take anyway.

    This function introduces a binary mask M that zeros out such tokens/sequences:

        M_{i,t} = 0  if  (A_{i,t} < 0) AND (KL(π_old || π_θ)_{i,t} > δ)
        M_{i,t} = 1  otherwise

    Intuition: "Learn from your own mistakes, not from irrelevant errors."
    Only mask negative-advantage tokens where the policy has already moved away
    from the old behavior. Positive-advantage tokens are never masked.
    ...
    Args:
        old_log_prob: Log-probabilities under the old (rollout) policy,
            shape (batch_size, response_length).
        log_prob: Log-probabilities under the current policy,
            shape (batch_size, response_length).
        advantages: Advantage estimates, shape (batch_size, response_length).
        response_mask: Binary mask for valid tokens, shape (batch_size, response_length).
        kl_threshold: δ — KL divergence threshold above which negative-advantage
            tokens are masked. A larger δ allows more off-policy tolerance.
            Typical range: 1.0–5.0. Default: 3.0.
        mask_granularity: Masking granularity level.
            - "token": Apply mask at token level (per-token KL check).
            - "sequence": Apply mask at sequence level (if any token in the
              sequence triggers the mask, the entire sequence is masked).
            Default: "token".

    Returns:
        Tuple of:
            offpolicy_mask: Binary mask (1=keep, 0=mask), shape (batch_size, response_length).
                Multiply with response_mask to get the effective training mask.
            metrics: Dictionary of diagnostic metrics.
    """
    if mask_granularity not in ("token", "sequence"):
        raise ValueError(f"mask_granularity must be 'token' or 'sequence', got '{mask_granularity}'")

    neg_advantage_mask = advantages < 0

    approx_kl = old_log_prob - log_prob
    approx_kl = torch.clamp(approx_kl, min=-20.0, max=20.0)

    high_kl_mask = approx_kl > kl_threshold

    mask_out = neg_advantage_mask & high_kl_mask

    if mask_granularity == "sequence":
        seq_has_masked = mask_out & response_mask.bool()
        seq_mask_out = seq_has_masked.any(dim=-1, keepdim=True).expand_as(response_mask)
        mask_out = seq_mask_out

    offpolicy_mask = (~mask_out).float()

    metrics: dict[str, float] = {}

    total_valid = response_mask.sum().clamp(min=1).item()
    neg_adv_valid = (neg_advantage_mask & response_mask.bool()).sum().item()
    high_kl_valid = (high_kl_mask & response_mask.bool()).sum().item()
    masked_valid = (mask_out & response_mask.bool()).sum().item()

    metrics["offpolicy_seq_mask/masked_fraction"] = masked_valid / max(total_valid, 1e-8)
    metrics["offpolicy_seq_mask/neg_adv_fraction"] = neg_adv_valid / max(total_valid, 1e-8)
    metrics["offpolicy_seq_mask/high_kl_fraction"] = high_kl_valid / max(total_valid, 1e-8)
    metrics["offpolicy_seq_mask/kl_threshold"] = kl_threshold
    metrics["offpolicy_seq_mask/mean_kl"] = verl_F.masked_mean(approx_kl, response_mask).item()
    metrics["offpolicy_seq_mask/max_kl"] = approx_kl.masked_fill(~response_mask.bool(), float("-inf")).max().item()

    seq_masked_out = mask_out & response_mask.bool()
    if seq_masked_out.any():
        seq_has_any_masked = seq_masked_out.any(dim=-1)
        seq_valid = (response_mask.sum(dim=-1) > 0)
        metrics["offpolicy_seq_mask/seq_masked_fraction"] = (
            seq_has_any_masked[seq_valid].float().mean().item()
            if seq_valid.any()
            else 0.0
        )
    else:
        metrics["offpolicy_seq_mask/seq_masked_fraction"] = 0.0

    return offpolicy_mask.detach(), metrics
...
def compute_policy_loss_vanilla(
    ...
    offpolicy_mask_metrics = {}

    use_offpolicy_seq_mask = config.policy_loss.get("use_offpolicy_seq_mask", False) if config.policy_loss else False
    if use_offpolicy_seq_mask:
        offpolicy_kl_threshold = config.policy_loss.get("offpolicy_seq_mask_kl_threshold", 3.0)
        offpolicy_mask_granularity = config.policy_loss.get("offpolicy_seq_mask_granularity", "token")
        offpolicy_mask, offpolicy_mask_metrics = compute_offpolicy_seq_mask(
            old_log_prob=old_log_prob,
            log_prob=log_prob,
            advantages=advantages,
            response_mask=response_mask,
            kl_threshold=offpolicy_kl_threshold,
            mask_granularity=offpolicy_mask_granularity,
        )
        effective_mask = response_mask * offpolicy_mask
    ...
    pg_metrics.update(offpolicy_mask_metrics)
...
def kl_penalty(
    ...
    """
    forward_score = kl_penalty_forward(logprob, ref_logprob, kl_penalty.rstrip("+") if kl_penalty.endswith("+") else kl_penalty)
