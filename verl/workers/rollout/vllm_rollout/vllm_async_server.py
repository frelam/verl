...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        if full_vocab is not None:
            if not (self._full_vocab_export_config and self._full_vocab_export_config.get("enabled")):
                raise ValueError(
            ...
            if self._disaggregation_role != "null":
                raise ValueError(
                    f"full-vocab hidden export is incompatible with PD disaggregation "
                    f"(role={self._disaggregation_role!r}, request_id={request_id}): the capture hook "
                    "needs the complete prefill logits pass on a single engine."
                )
        ...
        if full_vocab is None:
            ...
            # artifact, not in prompt logprobs: the prefill runs with
            # prompt_logprobs=0 only to force per-position logits (which drives
            # the capture hook), so there is nothing to extract for them.
            extract_prompt_logprobs(
                output=final_res,
    ...
    async def _export_full_vocab_hidden(self, request_id: str, full_vocab: dict[str, Any], seq_len: int) -> dict:
        ...
        try:
            ...
            raise RuntimeError(
                f"full-vocab export: fetch_captured_hidden failed (request_id={request_id}, "
                f"full_vocab={full_vocab})"
            ) from exc
    ...
    def _validate_configs(self) -> None:
        ...
        if self._full_vocab_export_config and self._full_vocab_export_config.get("enabled"):
            # Full-vocab hidden capture requires exactly one forward per request:
            # a single request in flight (max_num_seqs=1) and an unchunked prefill
            # (otherwise only the tail chunk's hidden would be captured, and the
            # seq_len check at export would reject every sample).
            if self.config.max_num_seqs != 1:
                raise ValueError(
                    f"full-vocab hidden export requires max_num_seqs=1 (one request per engine at a "
                    f"time, so the shared capture buffer is unambiguous), got {self.config.max_num_seqs}."
                )
            if self.config.enable_chunked_prefill:
                raise ValueError(
                    "full-vocab hidden export requires enable_chunked_prefill=False: a chunked "
                    "prefill runs the LogitsProcessor once per chunk and only the last chunk's "
                    "hidden state would be captured."
                )
            if self.config.max_num_batched_tokens < self.config.max_model_len:
                raise ValueError(
                    f"full-vocab hidden export requires max_num_batched_tokens >= max_model_len "
                    f"({self.config.max_model_len}), got {self.config.max_num_batched_tokens}: the v1 "
                    "scheduler would otherwise split a long prefill and silently truncate the capture."
                )
