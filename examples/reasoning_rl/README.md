...
| `extra_info` | dict | 键固定：`split(str)` `index(int)` `task_id(str)` `domain(str)` `source(str)` `difficulty(str)` `prior_solve_rate(float)` `seed(int,-1=无)`；`difficulty_tag.py` 追加 `pass_rate(float)`（mix.py 对未打标池自动补 -1.0）。reasoning_gym 行的 `seed` 是奖励侧用库自带 verifier 复现题目的唯一依据，勿丢 |
...
先跑一次「数据 ↔ reward」自检（不需要 GPU）：把每行自己的 ground_truth 当答案喂回 reward 函数（echo），
正常应 100% 命中；某个源 <100% 就说明该源的数据或 verifier 有问题，不必先怀疑模型：

```bash
python3 examples/reasoning_rl/scripts/check_reward.py --data_file $DATA_DIR/val.parquet
python3 examples/reasoning_rl/scripts/check_reward.py --data_file $DATA_DIR/val.parquet \
...
`code_*`（要沙箱）和 `if_*`（constraint JSON 不是答案）不做 echo；`if_*` 改为打印
**constraint coverage** —— 一行里只要有一个 reward 不认识的 instruction id，binary 判分下
这行就永远 0，所以这个比例必须是 100%（见 DESIGN.md §6 "Instruction-following 约束覆盖"）。
...
| `val-core/math_*` 偏低 | 先自检排除 reward：Big-Math-RL-Verified 2,000 条、DAPO-Math-17k 2,000 条按 prompt 的 `\boxed{}` 回灌全部判 1.0（DAPO 用 prompt 原生的 `Answer: $X$` 也全部 1.0） | 不是 reward 问题；`check_reward.py --data_file $DATA_DIR/val.parquet --data_source math_bigmath`（期望 100%） |
| `val-core/if_nemotron/*` 偏低 | 数据用了 48 种 instruction id，而旧 `if_verifier.py` 只实现 30 个：**77%（35,713/46,391）的行带无法评估的 id，binary 判分下永远 0**；另有 3 处 kwargs 名不匹配（`let_*`/`capital_*`/`nth_paragraph`）与 1 处恒真（`number_placeholders`） | 已修：`if_verifier.py` 覆盖官方 registry 全部 54 个 id，逐条对齐 `verifiable_instructions`（DESIGN.md §6 "Instruction-following 约束覆盖"，覆盖率 23%→100%）。**建议 `pip install langdetect`**（30 种目标语言里西语/越南语等拉丁字母语言没有它无法识别）；自检用 `check_reward.py --data_source if_nemotron` 看 constraint coverage（期望 100%） |
| `val-core/code_*` 偏低 | 用各数据集自带 accepted solution 回灌：apps 400 条 378 条满分（94.5%）、deepcoder/taco 250 条 242 条（96.8%）、code_contests 342 条里 231 条有 Python 参考解、115/152 满分（76%）。失败行主要是**数据固有噪声**：多解只存一解（Codeforces special judge）、expected output 的小数位数/大小写/列对齐与参考解不同、参考解自带 debug print；另有 `sandbox_fusion` wrapper 的 fn_name 解析 bug（`class Solution: def search` 会调到 `re.search`）与行尾空格比较过严 | 已修（verl 核心 `verl/utils/reward_score/sandbox_fusion/utils.py`）：① wrapper 用 `_PREEXISTING_GLOBALS` 快照优先用户定义名（`tests/utils/reward_score/test_sandbox_fusion_fn_name_wrapper.py` 覆盖）；② stdout 比较改为按行 rstrip（对齐判题惯例，恢复 ~2–3% 行）。剩下的多解/浮点格式噪声改不了，只能算 attention/难度问题，或换 special-judge 数据 |
| 重建 code 数据时报 `Config name is missing` / `load_dataset() got an unexpected keyword argument 'trust_remote_code'` / DeepCoder 全部 skipped | `datasets` 4.x：`agentica-org/DeepCoder-Preview-Dataset` 有 4 个 config（必须显式指定，`lcbv5`/`taco`/`primeintellect` 各只有部分 train），且 4.x 不再接受 `trust_remote_code`；另外 `lcbv5`/`codeforces` 的 `tests` 是 `[{input,output,testtype}]` 列表，`normalize_in_outs` 只认 `{"inputs","outputs"}` → 整源被静默丢弃 | 需改 `scripts/to_parquet_code.py`（本次未动，避免改变你现有 mix 配比）：给 `load_deepcoder` 显式传 config 并把 `lcbv5`/`codeforces` 的 tests 列表转成 `inputs/outputs`；`trust_remote_code` 只在 `load_dataset` 签名支持时才传。**若你的 parquet 里已有 `code_deepcoder` 行，说明当时用的是旧版脚本/旧 schema，重建前务必 diff 一下 `mix_stats.json`** |
