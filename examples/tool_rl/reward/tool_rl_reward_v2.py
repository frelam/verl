"""V2 Tool RL reward for verl — 3 dimensions, rule-based (label mode only).

Variant of ``tool_rl_reward.py`` for use with data generated by
``prepare_data_v2.py``: reads the ``answerable_direct`` /
``augment_detail`` tags robustly across the parquet storage variants
(bool, JSON ``"true"``/``"false"``, ``"True"``-style str casts).  Pair it
with the v2 data script via
``reward.custom_reward_function.path=.../tool_rl_reward_v2.py``.

Original module (kept unchanged for the legacy mix): tool_rl_reward.py,
ported from slime ``examples/tool_rl/reward/reward.py``.
