"""V2 reward/data tests — missing_info ask-negatives + storage-safe tags.

Mirrors ``test_tool_rl_reward.py`` with imports redirected to the v2
pair (``prepare_data_v2`` / ``tool_rl_reward_v2``).  Run from the repo
root:

.. code-block:: bash

    python -m pytest examples/tool_rl/reward/test_tool_rl_reward_v2.py -v
"""
...
from examples.tool_rl.reward.tool_rl_reward_v2 import compute_score  # noqa: E402
...
def test_self_computable_tool_regex():
    from examples.tool_rl.prepare_data_v2 import _SELF_COMPUTABLE_TOOL_RE
...
def test_self_computable_query_regex():
    from examples.tool_rl.prepare_data_v2 import _SELF_COMPUTABLE_QUERY_RE
...
def test_tag_answerable_direct_negatives():
    from examples.tool_rl.prepare_data_v2 import tag_answerable_direct_negatives
...
def test_desc_replace_tags_self_computable_tool():
    ...
    from examples.tool_rl.prepare_data_v2 import _augment_desc_replace
...
def _task_with_arg(value, query="Book a table at the bistro for me."):
    from examples.tool_rl.prepare_data_v2 import _format_gt, _normalize_tools
...
def test_missing_info_converts_unknowable_slot():
    ...
    from examples.tool_rl.prepare_data_v2 import _augment_missing_info
...
def test_augment_missing_info_tasks_targeted_ratio():
    ...
    from examples.tool_rl.prepare_data_v2 import augment_missing_info_tasks

    # ratio = desired share of the FINAL negative pool. One original negative
    # + one convertible positive: k = 0.5/(1-0.5) * 1 = 1 conversion makes
    # missing_info 1/2 of the negatives.
    bad = _task_with_arg("John")
    original = _task_with_arg("Smith", query="q")
    original["metadata"]["ground_truth"] = []
    original["metadata"]["augmented"] = "no_tools"
    good = _task_with_arg("Smith", query="Book a table for Smith.")
    n = augment_missing_info_tasks(
        [bad, original, good], ratio=0.5, rng=random.Random(0)
    )
    assert n == 1
...
def test_split_group_key():
    from examples.tool_rl.prepare_data_v2 import _split_group_key
...
def test_group_aware_split_keeps_conversations_together():
    from examples.tool_rl.prepare_data_v2 import group_aware_split
...
def test_load_toolace_recovers_ground_truth(monkeypatch):
    from examples.tool_rl import prepare_data_v2
    ...
    tasks = prepare_data_v2.load_toolace(max_samples=10)
    assert len(tasks) == 1
...
def test_load_toolace_guard_raises_on_unparsed_calls(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="call-shaped"):
        prepare_data_v2.load_toolace(max_samples=10)
